...
def extract_chains(links, pins_index):
    ...
    if not len(inputs) != len(outputs):
        raise ValueError(
            "The same number of input and ouput pins should be used from the driver.")

    if not (2 <= len(inputs) <= 16):
        raise ValueError(
            f"The number of chains should be between 2 and 16, currently {len(input)}.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for chain in chains:
            assert check_valid_chain(
                chain), "Chains should start and end at the driver."
        print("Chains start and end at the driver: check")

        assert check_all_pins_routed(
            chains, pins_to_route), "All pins should be routed exactly once."
